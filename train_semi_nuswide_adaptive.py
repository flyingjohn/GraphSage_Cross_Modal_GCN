...
#learning rate
flags.DEFINE_float('lr_img', 0.00005, 'Initial learning rate.')
flags.DEFINE_float('lr_txt', 0.00005, 'Initial learning rate.')
...
#other setting
flags.DEFINE_integer('epochs', 200, 'Number of epochs to train.') # 100 for 10-bit
# flags.DEFINE_integer('epochs', 300, 'Number of epochs to train.') # 300 for others
...
placeholders = {
    ...
    'lr_img': tf.placeholder(tf.float32, name='image_learning_rate'),
    'lr_txt':tf.placeholder(tf.float32, name='text_learning_rate'),
    'lr_cl':tf.placeholder(tf.float32, name='classifier_learning_rate')
}
...
# img optimizer
img_optimizer = tf.train.AdamOptimizer(learning_rate=placeholders['lr_img'])
img_vars = img_model.vars
img_op = img_optimizer.minimize(img_loss, var_list=img_vars)
#
# # txt optimizer
txt_optimizer = tf.train.AdamOptimizer(learning_rate=placeholders['lr_txt'])
txt_vars = txt_model.vars
txt_op = txt_optimizer.minimize(txt_loss, var_list=txt_vars)

# # label classifier optimizer
lc_optimizer = tf.train.AdamOptimizer(learning_rate=placeholders['lr_cl'])
...
# combine optimize
total_optimizer = tf.train.AdamOptimizer(learning_rate=placeholders['lr_img'])
...
if FLAGS.train:
    ...
    lr = np.linspace(0.00001, 0.000001, num=FLAGS.epochs, endpoint=True, retstep=False, dtype=None)
    # Init variables
    ...
    for epoch in range(FLAGS.epochs):
        ...
        while not minibatch.end():
            ...
            feed_dict.update({placeholders['lr_img']: FLAGS.lr_img})
            feed_dict.update({placeholders['lr_cl']: FLAGS.lr_label})
...
'''map per category'''
balance_dir = './data/nuswide-balance'
with open(balance_dir + '/train_index.pkl', 'r') as f:
    train_index = cPickle.load(f)
with open(balance_dir + '/test_index.pkl', 'r') as f:
    test_index = cPickle.load(f)
test_img_feats_trans_binary_ = test_img_feats_trans_binary[test_index]
test_txt_vecs_trans_binary_ = test_txt_vecs_trans_binary[test_index]
train_img_feats_trans_binary_ = train_img_feats_trans_binary[train_index]
train_txt_vecs_trans_binary_ = train_txt_vecs_trans_binary[train_index]
test_labels_ = test_labels[test_index]
train_labels_ = train_labels[train_index]
mapi2t_category = calc_map_category(test_img_feats_trans_binary_, train_txt_vecs_trans_binary_, test_labels_, train_labels_)
mapt2i_category = calc_map_category(test_txt_vecs_trans_binary_, train_img_feats_trans_binary_, test_labels_, train_labels_)
print(mapi2t_category)
print(mapt2i_category)

# result_save_path = './data/'+FLAGS.dataset+'/'+'result/'+datetime.now().strftime("%d-%h-%m-%s")+'_'+str(FLAGS.hash_bit)+'_bit_result.pkl'
result_save_path = './data/'+FLAGS.dataset+'/'+'result/'+'semi_adaptive_'+str(FLAGS.hash_bit)+'_'+str(FLAGS.percentage)+'_bit.pkl'
result = dict()
...
result['mapi2t_category'] = mapi2t_category
result['mapt2i_category'] = mapt2i_category
